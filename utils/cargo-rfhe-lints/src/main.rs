...
fn get_supported_rustc_version() -> &'static str {
    const TOOLCHAIN_FILE: &str = include_str!("../../cargo-rfhe-lints-inner/rust-toolchain.toml");
...
fn main() {
    ...
    if let Err(err) = Command::new("cargo")
        ...
        .arg("rfhe-lints-inner")
        .args(&cargo_args)
        .spawn()
        .and_then(|mut child| child.wait())
    {
        eprintln!(
            "Command `cargo {toolchain} rfhe-lints-inner {}` failed: {err:?}",
            cargo_args.join(" "),
        );
        exit(1);
    }
