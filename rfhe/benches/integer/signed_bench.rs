...
use rand::prelude::*;
use std::env;
use rfhe::integer::keycache::KEY_CACHE;
use rfhe::integer::prelude::*;
use rfhe::integer::{IntegerKeyKind, RadixCiphertext, ServerKey, SignedRadixCiphertext, I256};
use rfhe::keycache::NamedParam;

fn gen_random_i256(rng: &mut ThreadRng) -> I256 {
    let clearlow = rng.gen::<u128>();
    let clearhigh = rng.gen::<u128>();

    rfhe::integer::I256::from((clearlow, clearhigh))
}
...
//
// rand::distributions::Distribution can't be implemented in rfhe sources
...
mod cuda {
    ...
    use rfhe::core_crypto::gpu::CudaStreams;
    use rfhe::integer::gpu::ciphertext::boolean_value::CudaBooleanBlock;
    use rfhe::integer::gpu::ciphertext::{CudaSignedRadixCiphertext, CudaUnsignedRadixCiphertext};
    use rfhe::integer::gpu::server_key::CudaServerKey;
        ...
        for (param, num_block, bit_size) in ParamsAndNumBlocksIter::default() {
            ...
            bench_group.bench_function(&bench_id, |b| {
                ...
                let encrypt_two_values = || {
                    let clearlow = rng.gen::<u128>();
                    let clearhigh = rng.gen::<u128>();
                    let clear_0 = rfhe::integer::I256::from((clearlow, clearhigh));
                    let ct_0 = cks.encrypt_signed_radix(clear_0, num_block);

                    let clearlow = rng.gen::<u128>();
                    let clearhigh = rng.gen::<u128>();
                    let clear_1 = rfhe::integer::I256::from((clearlow, clearhigh));
        ...
        for (param, num_block, bit_size) in ParamsAndNumBlocksIter::default() {
            ...
            bench_group.bench_function(&bench_id, |b| {
                ...
                let encrypt_one_value = || {
                    let clearlow = rng.gen::<u128>();
                    let clearhigh = rng.gen::<u128>();
                    let clear = rfhe::integer::I256::from((clearlow, clearhigh));
        ...
        for (param, num_block, bit_size) in ParamsAndNumBlocksIter::default() {
            ...
            bench_group.bench_function(&bench_id, |b| {
                ...
                let encrypt_two_values = || {
                    ...
                    let clearlow = rng.gen::<u128>();
                    let clearhigh = rng.gen::<u128>();
                    let clear_1 = rfhe::integer::U256::from((clearlow, clearhigh));
                    let ct_1 = cks.encrypt_radix(clear_1, num_block);
    ...
    fn mul_signed_scalar(rng: &mut ThreadRng, _clear_bit_size: usize) -> ScalarType {
        loop {
            let clearlow = rng.gen::<u128>();
            let clearhigh = rng.gen::<u128>();
            let scalar = rfhe::integer::I256::from((clearlow, clearhigh));
...
fn main() {
    match env::var("__RFHE_RS_BENCH_OP_FLAVOR") {
        Ok(val) => {
            #[cfg(feature = "gpu")]
