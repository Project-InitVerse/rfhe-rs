...
use std::io::Write;
use std::path::Path;
use rfhe::core_crypto::prelude::*;
use rfhe::integer::key_switching_key::KeySwitchingKey;
use rfhe::integer::parameters::{
    IntegerCompactCiphertextListCastingMode, IntegerCompactCiphertextListUnpackingMode,
};
use rfhe::integer::{ClientKey, CompactPrivateKey, CompactPublicKey, ServerKey};
use rfhe::keycache::NamedParam;
use rfhe::shortint::parameters::classic::tuniform::p_fail_2_minus_64::ks_pbs::PARAM_MESSAGE_2_CARRY_2_KS_PBS_TUNIFORM_2M64;
use rfhe::shortint::parameters::compact_public_key_only::PARAM_PKE_MESSAGE_2_CARRY_2_KS_PBS_TUNIFORM_2M64;
use rfhe::shortint::parameters::key_switching::PARAM_KEYSWITCH_MESSAGE_2_CARRY_2_KS_PBS_TUNIFORM_2M64;
use rfhe::shortint::parameters::PBSParameters;
use rfhe::zk::{CompactPkeCrs, ZkComputeLoad};
        ...
        for bits in [640usize, 1280, 4096] {
            ...
            for compute_load in [ZkComputeLoad::Proof, ZkComputeLoad::Verify] {
                ...
                bench_group.bench_function(&bench_id, |b| {
                    b.iter(|| {
                        let _ct1 = rfhe::integer::ProvenCompactCiphertextList::builder(&pk)
    ...
    for (param_pke, param_casting, param_fhe) in [(
        ...
        for bits in [640usize, 1280, 4096] {
            ...
            for compute_load in [ZkComputeLoad::Proof, ZkComputeLoad::Verify] {
                ...
                let ct1 = rfhe::integer::ProvenCompactCiphertextList::builder(&pk)
                    .extend(messages.iter().copied())
                    .build_with_proof_packed(public_params, compute_load)
                    .unwrap();
