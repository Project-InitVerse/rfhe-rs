...
use rfhe::integer::{IntegerKeyKind, RadixCiphertext, RadixClientKey, ServerKey, U256};
...
fn gen_random_u256(rng: &mut ThreadRng) -> U256 {
    ...
    rfhe::integer::U256::from((clearlow, clearhigh))
}
        ...
        bench_group.bench_function(&bench_id, |b| {
            ...
            let encrypt_one_value = || {
                ...
                while carry_mod > 0 {
                    ...
                    let clearlow = rng.gen::<u128>();
                    let clearhigh = rng.gen::<u128>();
                    let clear_2 = rfhe::integer::U256::from((clearlow, clearhigh));
                    let ct_2 = cks.encrypt_radix(clear_2, num_block);
                    sks.unchecked_add_assign(&mut ct_0, &ct_2);
...
mod cuda {
    ...
    use rfhe::integer::gpu::ciphertext::CudaUnsignedRadixCiphertext;
        ...
        for (param, num_block, bit_size) in ParamsAndNumBlocksIter::default() {
            ...
            bench_group.bench_function(&bench_id, |b| {
                ...
                let encrypt_gpu = || {
                    let clearlow = rng.gen::<u128>();
                    let clearhigh = rng.gen::<u128>();
                    let clear_0 = rfhe::integer::U256::from((clearlow, clearhigh));
                    let ct_0 = cks.encrypt_radix(clear_0, num_block);
    ...
    fn bench_cuda_server_key_binary_scalar_function_clean_inputs<F, G>(
        ...
        F: Fn(&CudaServerKey, &mut CudaUnsignedRadixCiphertext, ScalarType, &CudaStreams),
        G: Fn(&mut ThreadRng, usize) -> ScalarType,
    {
        let mut bench_group = c.benchmark_group(bench_name);
        bench_group
