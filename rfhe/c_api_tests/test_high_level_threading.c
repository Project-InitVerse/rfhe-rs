 ...
 * each of them, it also uses RfheThreadingContext in order to limit
 * the number of threads the operation can use.
 *
 * The RfheThreadingContext is like a thread pool, so when created, it creates
 ...
 * the number of RfheThreadingContext that exist, and reuse them.
 *
 * Which this test does not do, meaning that if you modify
 * NUM_C_THREADS and/or NUM_RFHE_THREAD_PER_CONTEXT it might end up
 ...
 * be:
 * ` NUM_CPU + NUM_C_THREADS + (NUM_C_THREADS * NUM_RFHE_THREAD_PER_CONTEXT) + 1
 *  with
 *      - NUM_CPU: number of CPU threads (rfhe internally automatically creates them)
 */
...
// Number of thread each RfheThreadingContext will have
#define NUM_RFHE_THREAD_PER_CONTEXT 2

/* Struct used to pass to a RfheThreadingContext some data
...
int do_work_in_rfhe_threading_context(void *vdata) {
...
void *c_threads_callback(void *vdata) {
  ...
  rfhe_threading_context_run(request->context, do_work_in_rfhe_threading_context,
                             (void *)request->data);
...
void do_thing(const ClientKey *client_key, const ServerKey *server_key) {
  ...
  RfheThreadingContext *contexts[NUM_C_THREADS] = {NULL};
  for (size_t i = 0; i < NUM_C_THREADS; ++i) {
    status = rfhe_threading_context_create(NUM_RFHE_THREAD_PER_CONTEXT, &contexts[i]);
    assert(status == 0);

    status = rfhe_threading_context_set_server_key(contexts[i], server_key);
    assert(status == 0);
  }
  ...
  for (size_t i = 0; i < NUM_C_THREADS; ++i) {
    ...
    rfhe_threading_context_destroy(contexts[i]);
