...
assert_eq!(clear, "INITVERSE INITVERSE INITVERSE");
```
...
- Shifting by an encrypted number of characters: By treating the string as a `RadixCiphertext` we can use the rfhe-rs shifting operations, and then convert back to `FheString`.
