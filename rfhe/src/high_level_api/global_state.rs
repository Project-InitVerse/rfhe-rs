...
///
/// ```rust
/// use rfhe::{generate_keys, ConfigBuilder};
///
/// let config = ConfigBuilder::default().build();
/// let (client_key, server_key) = generate_keys(config);
///
/// rfhe::set_server_key(server_key);
...
/// use std::thread;
/// use rfhe;
/// use rfhe::ConfigBuilder;
///
/// let config = rfhe::ConfigBuilder::default().build();
/// let (client_key, server_key) = rfhe::generate_keys(config);
...
///     rfhe::set_server_key(server_key_2);
///     // Now, this thread we can do operations on homomorphic types
/// });
///
/// th2.join().unwrap();
