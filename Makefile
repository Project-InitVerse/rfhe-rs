...
MIN_RUST_VERSION:=$(shell grep '^rust-version[[:space:]]*=' rfhe/Cargo.toml | cut -d '=' -f 2 | xargs)
AVX512_SUPPORT?=OFF
WASM_RUSTFLAGS:=
BIG_TESTS_INSTANCE?=FALSE
GEN_KEY_CACHE_MULTI_BIT_ONLY?=FALSE
GEN_KEY_CACHE_COVERAGE_ONLY?=FALSE
PARSE_INTEGER_BENCH_CSV_FILE?=rfhe_rs_integer_benches.csv
FAST_TESTS?=FALSE
FAST_BENCH?=FALSE
NIGHTLY_TESTS?=FALSE
...
# rfhe-cuda-backend
RFHECUDA_SRC=backends/rfhe-cuda-backend/cuda
RFHECUDA_BUILD=$(RFHECUDA_SRC)/build
...
--exclude-files rfhe/benches/boolean/* \
--exclude-files rfhe/benches/core_crypto/* \
--exclude-files rfhe/benches/shortint/* \
--exclude-files rfhe/benches/integer/* \
--exclude-files rfhe/benches/* \
--exclude-files rfhe/examples/regex_engine/* \
--exclude-files rfhe/examples/utilities/*
endef
...
.PHONY: install_rfhe_lints # Install custom rfhe-rs lints
install_rfhe_lints:
	(cd utils/cargo-rfhe-lints-inner && cargo install --path .) && \
	cd utils/cargo-rfhe-lints && cargo install --path .
...
fmt_js: check_nvm_installed
	...
	$(MAKE) -C rfhe/web_wasm_parallel_tests fmt

.PHONY: fmt_gpu # Format rust and cuda code
fmt_gpu: install_rs_check_toolchain
	cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" fmt
	cd "$(RFHECUDA_SRC)" && ./format_rfhe_cuda_backend.sh

.PHONY: fmt_c_tests # Format c tests
fmt_c_tests:
	find rfhe/c_api_tests/ -regex '.*\.\(cpp\|hpp\|cu\|c\|h\)' -exec clang-format -style=file -i {} \;
...
check_fmt_c_tests:
	find rfhe/c_api_tests/ -regex '.*\.\(cpp\|hpp\|cu\|c\|h\)' -exec clang-format --dry-run --Werror -style=file {} \;

.PHONY: check_fmt_gpu # Check rust and cuda code format
check_fmt_gpu: install_rs_check_toolchain
	cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" fmt --check
	cd "$(RFHECUDA_SRC)" && ./format_rfhe_cuda_backend.sh -c
...
check_fmt_js: check_nvm_installed
	...
	nvm install $(NODE_VERSION) && \
	nvm use $(NODE_VERSION) && \
	$(MAKE) -C rfhe/web_wasm_parallel_tests check_fmt

.PHONY: clippy_gpu # Run clippy lints on rfhe with "gpu" enabled
clippy_gpu: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" clippy \
		--features=$(TARGET_ARCH_FEATURE),boolean,shortint,integer,internal-keycache,gpu \
		--all-targets \
		-p $(RFHE_SPEC) -- --no-deps -D warnings
...
clippy_trivium: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" clippy --all-targets \
		-p rfhe-trivium -- --no-deps -D warnings
...
.PHONY: clippy_zk_pok # Run clippy lints on rfhe-zk-pok
clippy_zk_pok: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" clippy --all-targets \
		-p rfhe-zk-pok -- --no-deps -D warnings
...
.PHONY: clippy_cuda_backend # Run clippy lints on the rfhe-cuda-backend
clippy_cuda_backend: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" clippy --all-targets \
		-p rfhe-cuda-backend -- --no-deps -D warnings

.PHONY: rfhe_lints # Run custom rfhe-rs lints
rfhe_lints: install_rfhe_lints
	cd rfhe && RUSTFLAGS="$(RUSTFLAGS)" cargo rfhe-lints \
		--features=$(TARGET_ARCH_FEATURE),boolean,shortint,integer -- -D warnings

.PHONY: build_core # Build core_crypto without experimental features
build_core: install_rs_build_toolchain install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) build --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE) -p $(RFHE_SPEC)
	@if [[ "$(AVX512_SUPPORT)" == "ON" ]]; then \
		RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_CHECK_TOOLCHAIN) build --profile $(CARGO_PROFILE) \
			--features=$(TARGET_ARCH_FEATURE),$(AVX512_FEATURE) -p $(RFHE_SPEC); \
	fi
...
build_core_experimental: install_rs_build_toolchain install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) build --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),experimental -p $(RFHE_SPEC)
	@if [[ "$(AVX512_SUPPORT)" == "ON" ]]; then \
		RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_CHECK_TOOLCHAIN) build --profile $(CARGO_PROFILE) \
			--features=$(TARGET_ARCH_FEATURE),experimental,$(AVX512_FEATURE) -p $(RFHE_SPEC); \
	fi

.PHONY: build_boolean # Build with boolean enabled
build_boolean: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) build --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),boolean -p $(RFHE_SPEC) --all-targets

.PHONY: build_shortint # Build with shortint enabled
build_shortint: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) build --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),shortint -p $(RFHE_SPEC) --all-targets

.PHONY: build_integer # Build with integer enabled
build_integer: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) build --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),integer -p $(RFHE_SPEC) --all-targets

.PHONY: build_rfhe_full # Build with boolean, shortint and integer enabled
build_rfhe_full: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) build --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),boolean,shortint,integer -p $(RFHE_SPEC) --all-targets

.PHONY: build_rfhe_coverage # Build with test coverage enabled
build_rfhe_coverage: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS) --cfg tarpaulin" cargo $(CARGO_RS_BUILD_TOOLCHAIN) build --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),boolean,shortint,integer,internal-keycache -p $(RFHE_SPEC) --tests
...
symlink_c_libs_without_fingerprint:
	@./scripts/symlink_c_libs_without_fingerprint.sh \
		...
		--lib-name rfhe-c-api-dynamic-buffer
...
build_web_js_api_parallel: install_rs_check_toolchain install_wasm_pack
	...
	find pkg/snippets -type f -iname workerHelpers.worker.js -exec sed -i "s|from '..\/..\/..\/';|from '..\/..\/..\/rfhe.js';|" {} \;
...
test_core_crypto: install_rs_build_toolchain install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),experimental,zk-pok -p $(RFHE_SPEC) -- core_crypto::
	@if [[ "$(AVX512_SUPPORT)" == "ON" ]]; then \
		RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_CHECK_TOOLCHAIN) test --profile $(CARGO_PROFILE) \
			--features=$(TARGET_ARCH_FEATURE),experimental,zk-pok,$(AVX512_FEATURE) -p $(RFHE_SPEC) -- core_crypto::; \
	fi
...
test_core_crypto_cov: install_rs_build_toolchain install_rs_check_toolchain install_tarpaulin
	...
	@if [[ "$(AVX512_SUPPORT)" == "ON" ]]; then \
		RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_CHECK_TOOLCHAIN) tarpaulin --profile $(CARGO_PROFILE) \
			...
			-p $(RFHE_SPEC) -- -Z unstable-options --report-time core_crypto::; \
	fi

.PHONY: test_cuda_backend # Run the internal tests of the CUDA backend
test_cuda_backend:
	mkdir -p "$(RFHECUDA_BUILD)" && \
		cd "$(RFHECUDA_BUILD)" && \
		cmake .. -DCMAKE_BUILD_TYPE=Release -DRFHE_CUDA_BACKEND_BUILD_TESTS=ON && \
		"$(MAKE)" -j "$(CPU_COUNT)" && \
		"$(MAKE)" test
...
test_core_crypto_gpu: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),gpu -p $(RFHE_SPEC) -- core_crypto::gpu::
...
test_integer_gpu: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),integer,gpu -p $(RFHE_SPEC) -- integer::gpu::server_key:: --test-threads=6
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --doc --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),integer,gpu -p $(RFHE_SPEC) -- integer::gpu::server_key::
...
test_unsigned_integer_gpu_ci: install_rs_check_toolchain install_cargo_nextest
	...
	NIGHTLY_TESTS="$(NIGHTLY_TESTS)" \
		./scripts/integer-tests.sh --rust-toolchain $(CARGO_RS_CHECK_TOOLCHAIN) \
		--cargo-profile "$(CARGO_PROFILE)" --backend "gpu" \
		--unsigned-only --rfhe-package "$(RFHE_SPEC)"
...
test_signed_integer_gpu_ci: install_rs_check_toolchain install_cargo_nextest
	...
	NIGHTLY_TESTS="$(NIGHTLY_TESTS)" \
		./scripts/integer-tests.sh --rust-toolchain $(CARGO_RS_CHECK_TOOLCHAIN) \
		--cargo-profile "$(CARGO_PROFILE)" --backend "gpu" \
		--signed-only --rfhe-package "$(RFHE_SPEC)"
...
test_boolean: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),boolean -p $(RFHE_SPEC) -- boolean::
...
test_boolean_cov: install_rs_check_toolchain install_tarpaulin
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_CHECK_TOOLCHAIN) tarpaulin --profile $(CARGO_PROFILE) \
		...
		-p $(RFHE_SPEC) -- -Z unstable-options --report-time boolean::
...
test_shortint_ci: install_rs_build_toolchain install_cargo_nextest
	...
	FAST_TESTS="$(FAST_TESTS)" \
		...
		--cargo-profile "$(CARGO_PROFILE)" --rfhe-package "$(RFHE_SPEC)"
...
test_shortint_multi_bit_ci: install_rs_build_toolchain install_cargo_nextest
	...
	FAST_TESTS="$(FAST_TESTS)" \
		./scripts/shortint-tests.sh --rust-toolchain $(CARGO_RS_BUILD_TOOLCHAIN) \
		--cargo-profile "$(CARGO_PROFILE)" --multi-bit --rfhe-package "$(RFHE_SPEC)"

.PHONY: test_shortint # Run all the tests for shortint
test_shortint: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),shortint,internal-keycache -p $(RFHE_SPEC) -- shortint::
...
test_shortint_cov: install_rs_check_toolchain install_tarpaulin
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_CHECK_TOOLCHAIN) tarpaulin --profile $(CARGO_PROFILE) \
		...
		-p $(RFHE_SPEC) -- -Z unstable-options --report-time shortint::
...
test_safe_deserialization: install_rs_build_toolchain install_cargo_nextest
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),boolean,shortint,integer,internal-keycache -p $(RFHE_SPEC) -- safe_deserialization::

.PHONY: test_integer # Run all the tests for integer
test_integer: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),integer,internal-keycache -p $(RFHE_SPEC) -- integer::
...
test_integer_cov: install_rs_check_toolchain install_tarpaulin
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_CHECK_TOOLCHAIN) tarpaulin --profile $(CARGO_PROFILE) \
		...
		-p $(RFHE_SPEC) -- -Z unstable-options --report-time integer::

.PHONY: test_high_level_api # Run all the tests for high_level_api
test_high_level_api: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),boolean,shortint,integer,internal-keycache,zk-pok -p $(RFHE_SPEC) \
		-- high_level_api::

test_high_level_api_gpu: install_rs_build_toolchain install_cargo_nextest
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) nextest run --cargo-profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),integer,internal-keycache,gpu -p $(RFHE_SPEC) \
...
test_user_doc_gpu: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile $(CARGO_PROFILE) --doc \
		--features=$(TARGET_ARCH_FEATURE),boolean,shortint,integer,internal-keycache,gpu,zk-pok -p $(RFHE_SPEC) \
		-- test_user_docs::
...
.PHONY: test_zk_pok # Run rfhe-zk-pok tests
test_zk_pok: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile $(CARGO_PROFILE) \
		-p rfhe-zk-pok

.PHONY: test_versionable # Run tests for rfhe-versionable subcrate
...
doc: install_rs_check_toolchain
	...
	cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" doc \
		--features=$(TARGET_ARCH_FEATURE),boolean,shortint,integer,gpu,internal-keycache,experimental,zk-pok --no-deps -p $(RFHE_SPEC)
...
lint_doc: install_rs_check_toolchain
	...
	cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" doc \
		--features=$(TARGET_ARCH_FEATURE),boolean,shortint,integer,gpu,internal-keycache,experimental,zk-pok -p $(RFHE_SPEC) --no-deps
...
check_compile_tests_benches_gpu: install_rs_build_toolchain
	...
	mkdir -p "$(RFHECUDA_BUILD)" && \
		cd "$(RFHECUDA_BUILD)" && \
		cmake .. -DCMAKE_BUILD_TYPE=Debug -DRFHE_CUDA_BACKEND_BUILD_TESTS=ON -DRFHE_CUDA_BACKEND_BUILD_BENCHMARKS=ON && \
		"$(MAKE)" -j "$(CPU_COUNT)"

.PHONY: build_nodejs_test_docker # Build a docker image with tools to run nodejs tests for wasm API
build_nodejs_test_docker:
	DOCKER_BUILDKIT=1 docker build --build-arg RUST_TOOLCHAIN="$(RS_BUILD_TOOLCHAIN)" \
		-f docker/Dockerfile.wasm_tests --build-arg NODE_VERSION=$(NODE_VERSION) -t rfhe-wasm-tests .
...
test_nodejs_wasm_api_in_docker: build_nodejs_test_docker
	...
	docker run --rm "$${RUN_FLAGS}" \
		-v "$$(pwd)":/rfhe-wasm-tests/rfhe-rs \
		-v rfhe-rs-root-target-cache:/root/rfhe-rs-target \
		-v rfhe-rs-pkg-cache:/rfhe-wasm-tests/rfhe-rs/rfhe/pkg \
		-v rfhe-rs-root-cargo-registry-cache:/root/.cargo/registry \
		-v rfhe-rs-root-cache:/root/.cache \
		rfhe-wasm-tests /bin/bash -i -c 'make test_nodejs_wasm_api'

.PHONY: test_nodejs_wasm_api # Run tests for the nodejs on wasm API
test_nodejs_wasm_api: build_node_js_api
	cd rfhe/js_on_wasm_tests && npm run test

.PHONY: test_web_js_api_parallel # Run tests for the web wasm api
test_web_js_api_parallel: build_web_js_api_parallel
	$(MAKE) -C rfhe/web_wasm_parallel_tests test
...
test_web_js_api_parallel_ci: build_web_js_api_parallel
	...
	nvm install $(NODE_VERSION) && \
	nvm use $(NODE_VERSION) && \
	$(MAKE) -C rfhe/web_wasm_parallel_tests test-ci

.PHONY: no_rfhe_typo # Check we did not invert the h and f in rfhe
no_rfhe_typo:
	@./scripts/no_rfhe_typo.sh
...
bench_integer: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" __RFHE_RS_BENCH_OP_FLAVOR=$(BENCH_OP_FLAVOR) __RFHE_RS_FAST_BENCH=$(FAST_BENCH) \
	cargo $(CARGO_RS_CHECK_TOOLCHAIN) bench \
	--bench integer-bench \
	--features=$(TARGET_ARCH_FEATURE),integer,internal-keycache,nightly-avx512 -p $(RFHE_SPEC) --
...
bench_integer_gpu: install_rs_check_toolchain
	...
	cargo $(CARGO_RS_CHECK_TOOLCHAIN) bench \
	--bench integer-bench \
	--features=$(TARGET_ARCH_FEATURE),integer,gpu,internal-keycache,nightly-avx512 -p $(RFHE_SPEC) --

.PHONY: bench_integer_multi_bit # Run benchmarks for unsigned integer using multi-bit parameters
bench_integer_multi_bit: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" __RFHE_RS_BENCH_TYPE=MULTI_BIT \
	__RFHE_RS_BENCH_OP_FLAVOR=$(BENCH_OP_FLAVOR) __RFHE_RS_FAST_BENCH=$(FAST_BENCH) \
	cargo $(CARGO_RS_CHECK_TOOLCHAIN) bench \
	--bench integer-bench \
	--features=$(TARGET_ARCH_FEATURE),integer,internal-keycache,nightly-avx512 -p $(RFHE_SPEC) --
...
bench_unsigned_integer_multi_bit_gpu: install_rs_check_toolchain
	...
	cargo $(CARGO_RS_CHECK_TOOLCHAIN) bench \
	--bench integer-bench \
	--features=$(TARGET_ARCH_FEATURE),integer,gpu,internal-keycache,nightly-avx512 -p $(RFHE_SPEC) -- ::unsigned
...
bench_shortint: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" __RFHE_RS_BENCH_OP_FLAVOR=$(BENCH_OP_FLAVOR) \
	cargo $(CARGO_RS_CHECK_TOOLCHAIN) bench \
	--bench shortint-bench \
	--features=$(TARGET_ARCH_FEATURE),shortint,internal-keycache,nightly-avx512 -p $(RFHE_SPEC)
...
bench_shortint_multi_bit: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" __RFHE_RS_BENCH_TYPE=MULTI_BIT \
	__RFHE_RS_BENCH_OP_FLAVOR=$(BENCH_OP_FLAVOR) \
	cargo $(CARGO_RS_CHECK_TOOLCHAIN) bench \
	--bench shortint-bench \
	--features=$(TARGET_ARCH_FEATURE),shortint,internal-keycache,nightly-avx512 -p $(RFHE_SPEC) --
...
bench_boolean: install_rs_check_toolchain
	...
	--bench boolean-bench \
	--features=$(TARGET_ARCH_FEATURE),boolean,internal-keycache,nightly-avx512 -p $(RFHE_SPEC)

.PHONY: bench_pbs # Run benchmarks for PBS
bench_pbs: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_CHECK_TOOLCHAIN) bench \
	--bench pbs-bench \
	--features=$(TARGET_ARCH_FEATURE),boolean,shortint,internal-keycache,nightly-avx512 -p $(RFHE_SPEC)
...
bench_pbs_gpu: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" __RFHE_RS_FAST_BENCH=$(FAST_BENCH) cargo $(CARGO_RS_CHECK_TOOLCHAIN) bench \
	--bench pbs-bench \
	--features=$(TARGET_ARCH_FEATURE),boolean,shortint,gpu,internal-keycache,nightly-avx512 -p $(RFHE_SPEC)
...
bench_web_js_api_parallel: build_web_js_api_parallel
	$(MAKE) -C rfhe/web_wasm_parallel_tests bench
...
bench_web_js_api_parallel_ci: build_web_js_api_parallel
	source ~/.nvm/nvm.sh && \
	nvm use $(NODE_VERSION) && \
	$(MAKE) -C rfhe/web_wasm_parallel_tests bench-ci
...
gen_key_cache_core_crypto: install_rs_build_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --tests --profile $(CARGO_PROFILE) \
		--features=$(TARGET_ARCH_FEATURE),experimental,internal-keycache -p $(RFHE_SPEC) -- --nocapture \
...
pcc: no_rfhe_typo no_dbg_log check_fmt lint_doc check_md_docs_are_tested check_intra_md_links \
clippy_all rfhe_lints check_compile_tests

.PHONY: pcc_gpu # pcc stands for pre commit checks for GPU compilation
pcc_gpu: clippy_gpu clippy_cuda_backend check_compile_tests_benches_gpu

.PHONY: fpcc # pcc stands for pre commit checks, the f stands for fast
fpcc: no_rfhe_typo no_dbg_log check_fmt lint_doc check_md_docs_are_tested clippy_fast \
check_compile_tests
