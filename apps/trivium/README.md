# FHE boolean Trivium implementation using RFHE-rs
...
current scheme. This means that any user of this feature must also have the `rfhe-rs` crate as a dependency.


Example of a Rust main below:
```rust
use rfhe::{ConfigBuilder, generate_keys, FheBool};
use rfhe::prelude::*;
use rfhe_trivium::TriviumStream;
...
encrypted via rfhe and trivium. For convenience we also provide `trans_decrypt_64`, but this is of course the exact same function.
...
of rfhe-rs, so the syntax is a little bit different. It also implements the `TransCiphering` trait. For optimization purposes, it does not internally run on the same 
cryptographic parameters as the high level API of rfhe-rs. As such, it requires the usage of a casting key, to switch from one parameter space to another, which makes 
its setup a little more intricate.

Example code:
```rust
use rfhe::shortint::prelude::*;
use rfhe::shortint::CastingKey;
use rfhe::{ConfigBuilder, generate_keys, FheUint64};
use rfhe::prelude::*;
use rfhe_trivium::TriviumStreamShortint;
...
# FHE Kreyvium implementation using rfhe-rs crate
