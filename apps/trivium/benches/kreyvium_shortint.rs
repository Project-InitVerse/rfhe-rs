...
use rfhe::{generate_keys, ConfigBuilder, FheUint64};
use rfhe_trivium::{KreyviumStreamShortint, TransCiphering};

pub fn kreyvium_shortint_warmup(c: &mut Criterion) {
    let config = ConfigBuilder::default().build();
    let (hl_client_key, hl_server_key) = generate_keys(config);
    let underlying_ck: rfhe::shortint::ClientKey = (*hl_client_key.as_ref()).clone().into();
    let underlying_sk: rfhe::shortint::ServerKey = (*hl_server_key.as_ref()).clone().into();
