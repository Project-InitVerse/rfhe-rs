...
#[link(name = "rfhe_cuda_backend", kind = "static")]
extern "C" {
    ...
    /// - `lwe_array_out`: output batch of num_samples bootstrapped ciphertexts c = (a0,..an-1,b)
    ///   where n is the LWE dimension
    /// - `lut_vector`: should hold as many test vectors of size polynomial_size as there are input
    ///   ciphertexts, but actually holds `num_lut_vectors` vectors to reduce memory usage
    /// - `lut_vector_indexes`: stores the index corresponding to which test vector to use for each
    ///   sample in `lut_vector`
    /// - `lwe_array_in`: input batch of num_samples LWE ciphertexts, containing n mask values + 1
    ///   body value
    /// - `bootstrapping_key`: GGSW encryption of the LWE secret key sk1 under secret key sk2. bsk =
    ///   Z + sk1 H where H is the gadget matrix and Z is a matrix (k+1).l containing GLWE
    ///   encryptions of 0 under sk2. bsk is thus a tensor of size (k+1)^2.l.N.n where l is the
    ///   number of decomposition levels and k is the GLWE dimension, N is the polynomial size for
    ///   GLWE. The polynomial size for GLWE and the test vector are the same because they have to
    ///   be in the same ring to be multiplied.
    /// - `pbs_buffer`: a preallocated buffer to store temporary results
    /// - `lwe_dimension`: size of the Torus vector used to encrypt the input LWE ciphertexts -
    ///   referred to as n above (~ 600)
    /// - `glwe_dimension`: size of the polynomial vector used to encrypt the LUT GLWE ciphertexts -
    ///   referred to as k above. Only the value 1 is supported for this parameter.
    /// - `polynomial_size`: size of the test polynomial (test vector) and size of the GLWE
    ///   polynomial (~1024)
    ...
    /// bootstrapping:
    ///   - the kernel is templatized based on integer discretization and polynomial degree
    ///   - num_samples * level_count * (glwe_dimension + 1) blocks of threads are launched, where
    ///     each thread is going to handle one or more polynomial coefficients at each stage, for a
    ///     given level of decomposition, either for the LUT mask or its body:
    ...
    ///   - between each stage a synchronization of the threads is necessary (some synchronizations
    ///     happen at the block level, some happen between blocks, using cooperative groups).
    ///   - in case the device has enough shared memory, temporary arrays used for the different
    ///     stages (accumulators) are stored into the shared memory
    ///   - the accumulators serve to combine the results for all decomposition levels
    ///   - the constant memory (64K) is used for storing the roots of identity values for the FFT
    ...
    /// - `lwe_array_in`: input batch of num_samples LWE ciphertexts, containing n mask values + 1
    ///   body value
    /// - `bootstrapping_key`: GGSW encryption of elements of the LWE secret key as in the classical
    ///   PBS, but this time we follow Zhou's trick and encrypt combinations of elements of the key
    /// - `pbs_buffer`: a preallocated buffer to store temporary results
    /// - `lwe_dimension`: size of the Torus vector used to encrypt the input LWE ciphertexts -
    ///   referred to as n above (~ 600)
    /// - `glwe_dimension`: size of the polynomial vector used to encrypt the LUT GLWE ciphertexts -
    ...
    /// - `v_stream` is a void pointer to the Cuda stream to be used in the kernel launch
    /// - `gpu_index` is the index of the GPU to be used in the kernel launch
    /// - `lwe_array_out`: output batch of num_samples keyswitched ciphertexts c = (a0,..an-1,b)
    ///   where n is the output LWE dimension (lwe_dimension_out)
    ...
    /// - `v_stream` is a void pointer to the Cuda stream to be used in the kernel launch
    /// - `gpu_index` is the index of the GPU to be used in the kernel launch
    /// - `lwe_array_out` is an array of size `(input_lwe_dimension + 1) *
    ///   input_lwe_ciphertext_count` that should have been allocated on the GPU before calling this
    ///   function, and that will hold the result of the computation.
    /// - `lwe_array_in` is the LWE ciphertext vector used as input, it should have been allocated
    ///   and initialized before calling this function. It has the same size as the output array.
    ...
    /// - `lwe_array_in` is the LWE ciphertext vector used as input, it should have been allocated
    ///   and initialized before calling this function. It has the same size as the output array.
    /// - `plaintext_array_in` is the plaintext vector used as input, it should have been allocated
    ///   and initialized before calling this function. It should be of size
    ...
    /// - `lwe_array_in` is the LWE ciphertext vector used as input, it should have been allocated
    ///   and initialized before calling this function. It has the same size as the output array.
    /// - `cleartext_array_in` is the cleartext vector used as input, it should have been allocated
    ///   and initialized before calling this function. It should be of size
    ///   `input_lwe_ciphertext_count`.
    /// - `input_lwe_dimension` is the number of mask elements in the input and output LWE
    ///   ciphertext vectors
