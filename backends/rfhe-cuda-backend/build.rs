...
fn main() {
    ...
    // "make" command during macro expansions for RFHE-rs C API, crashing make for make < 4.4 and
    // thus crashing the build
    // On the other hand, this speeds up C API build greatly given we don't have macro expansions
    // in the CUDA backend so this skips the second compilation of RFHE-rs for macro inspection by
    // cbindgen
    if std::env::var("_CBINDGEN_IS_RUNNING").is_ok() {
        return;
    }

    println!("Build rfhe-cuda-backend");
    ...
    if env::consts::OS == "linux" {
        let output = Command::new("bash")
            ...
            .output()
            .expect("Failed to execute get_os_name.sh");
        ...
        if distribution != "Ubuntu\n" {
            println!(
                ...
                Only Ubuntu is supported by rfhe-cuda-backend at this time. Build may fail\n"
            );
        }
        let dest = cmake::build("cuda");
        println!("cargo:rustc-link-search=native={}", dest.display());
        println!("cargo:rustc-link-lib=static=rfhe_cuda_backend");
        ...
        panic!(
            "Error: platform not supported, rfhe-cuda-backend not built (only Linux is supported)"
        );
    }
}
