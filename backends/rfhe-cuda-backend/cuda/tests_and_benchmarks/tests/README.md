...
```bash
$ cd rfhe-rs/backends/rfhe-cuda-backend/cuda
$ mkdir build
$ cd build
```
...
`crfhe-rs/backends/rfhe-cuda-backend/cuda/build/tests/src`.

## How to Run Tests

To run tests, you can simply execute the `test_rfhe_cuda_backend` executable with no arguments:

```bash
$ tests/src/test_rfhe_cuda_backend
```
...
```bash
$ tests/src/test_rfhe_cuda_backend --gtest_filter=Bootstrap*
```
